...
export function StatusBar({
  ...
  return (
    <header className="flex h-9 shrink-0 items-center gap-3 border-b border-grid bg-void px-3 font-mono text-[10px] uppercase tracking-eyebrow text-ink-faint">
      ...
      <span className={live ? "text-gain" : "text-ink-faint"}>
        <span className={`mr-1 inline-block h-2 w-2 ${live ? "animate-livepulse bg-gain" : "bg-ink-faint"}`} />
        {live ? "LIVE" : "LATEST"}
      </span>
      <span className="hidden min-w-0 truncate md:inline">
        {session ? `${session.year} ${session.country_name} · ${session.session_name}` : "loading session"}
      </span>
